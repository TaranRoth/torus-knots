...
from sage.all import Matrix, ZZ, CC, RR, pi, I, exp

could_be_slice = True
from sage.all import (
    Matrix, ZZ, QQbar, pi, I, exp, RR, ComplexField
)

def tristram_levine_signature(link, omega):
    S = Matrix(ZZ, link.seifert_matrix())

    try:
        omega_exact = QQbar(omega)
        H = (1 - omega_exact) * S + (1 - omega_exact.conjugate()) * S.transpose()
        ev = H.eigenvalues()
        pos = sum(1 for x in ev if x > 0)
        neg = sum(1 for x in ev if x < 0)
    except (TypeError, ValueError):
        CC_high = ComplexField(100)
        omega_num = CC_high(omega)
        H = (1 - omega_num) * S + (1 - omega_num.conjugate()) * S.transpose()
        H = Matrix(CC_high, H)
        ev = H.eigenvalues()
        pos = sum(1 for x in ev if x.real() > 1e-8 and abs(x.imag()) < 1e-8)
        neg = sum(1 for x in ev if x.real() < -1e-8 and abs(x.imag()) < 1e-8)

    return pos - neg

def could_be_slice_tristram(link, num_samples=10):
    for k in range(1, num_samples + 1):
        theta = RR(k) / (num_samples + 1)
        omega = exp(2 * pi * I * theta)
        sig = tristram_levine_signature(link, omega)
        print(f"ω = exp(2πi * {theta:.4f}), signature = {sig}")
        if sig != 0:
            return False
    return True
...
if not valid_alexander(alexander):
    could_be_slice = False
print('Alexander: ' + str(alexander))
print('Maximal Crossing Number: ' + str(len(l.crossings)))
print('Could be Slice: ' + ('Yes' if could_be_slice else 'No'))
# print(str(l.PD_code()).replace('(', '[').replace(')', ']'))
dt = l.DT_code()
n = len(dt)
code = f"{n} " + " ".join(str(x) for x in dt)
print(str(code).replace(',',''))
print(l.PD_code())
print(could_be_slice_tristram(l))